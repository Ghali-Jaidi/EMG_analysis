...
arguments
    x {mustBeNumeric}
    fc (1,2) {mustBePositive} = default_emg_parameters().filter.bp_fc
    fs (1,1) {mustBePositive} = default_emg_parameters().fs
    order (1,1) {mustBeNonnegative, mustBeInteger} = default_emg_parameters().filter.bp_order
end
