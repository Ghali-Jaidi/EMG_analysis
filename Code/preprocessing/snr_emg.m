...
arguments
    ...
    win_ms (1,1) double {mustBePositive} = default_emg_parameters().snr_win_ms
    act_prc (1,1) double {mustBePositive} = default_emg_parameters().act_prc
    ...
    options.min_active_dur_ms (1,1) double {mustBePositive} = default_emg_parameters().min_active_dur_ms
    options.act_fuse_gap_ms   (1,1) double {mustBePositive} = default_emg_parameters().act_fuse_gap_ms
end
...
minAct = round((options.min_active_dur_ms/1000)*fs);
is_act = keep_long_runs(is_act, minAct);
is_act = fuse_masks(is_act, fs, options.act_fuse_gap_ms);
...
if ~isempty(options.xMG)
    ...
    minAct_MG = round((options.min_active_dur_ms/1000)*fs);
    is_act_MG = keep_long_runs(is_act_MG, minAct_MG);
    is_act_MG = fuse_masks(is_act_MG, fs, options.act_fuse_gap_ms);
