...
**Key Features:**
- Operates on the raw (unfiltered) 10 kHz signals for spectral fidelity
...
2. For each epoch:
   ...
   d. Causal bandpass & mean-square:
      - bpass_filt = butter_filter(signal, [100 500], fs, 4)
...
% Time-domain (Butterworth bandpass; signature: butter_filter(x, fc, fs, order)):
bpass_sig = butter_filter(signal, [f_low, f_high], fs, 4);
...
> Note on filter defaults: when `butter_filter` is called with no `fc`/`fs`/`order`,
> it now takes them from `config/default_emg_parameters.m` (`P.filter.bp_fc`,
> `P.fs`, `P.filter.bp_order`). The spectral routines above pass explicit band
> edges and order, so they are unaffected by the config defaults.

## Related Modules

- **Config** (`config/default_emg_parameters.m`): supplies default filter cutoffs/order/`fs`
- **Preprocessing** (`preprocessing/`): provides preprocessed signals
- **Detection** (`detection/`): detects events within bands
- **Visualization** (`visualization/`): visualization of spectra
