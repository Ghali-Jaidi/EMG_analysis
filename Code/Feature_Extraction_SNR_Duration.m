%% =========================
% Master analysis script - SNR and Duration Only
% (Simplified version without PNR and cross-correlation plots)
%% =========================
clear; clc;
...
%% ============================================================
%  PART 1 — Injured vs Uninjured comparison (SNR & Duration only)
%% ============================================================
if run_group_compare
    ...
    %% ---- Plot (SNR, Duration, Overlap only) ----
    ...
    figure('Name','Group comparison: SNR, Duration, Overlap');
    ...
    % Set bar colors to darker for better contrast
    b(1).FaceColor = colors.TA;
    b(2).FaceColor = colors.MG;
    errorbar(x{1}, M_SNR(:,1), SD_SNR(:,1), 'k', 'LineWidth', 2, 'CapSize', 12, 'LineStyle', 'none');
    errorbar(x{2}, M_SNR(:,2), SD_SNR(:,2), 'k', 'LineWidth', 2, 'CapSize', 12, 'LineStyle', 'none');
    ...
    b(1).FaceColor = colors.TA;
    b(2).FaceColor = colors.MG;
    errorbar(x{1}, M_DUR(:,1), SD_DUR(:,1), 'k', 'LineWidth', 2, 'CapSize', 12, 'LineStyle', 'none');
    errorbar(x{2}, M_DUR(:,2), SD_DUR(:,2), 'k', 'LineWidth', 2, 'CapSize', 12, 'LineStyle', 'none');
    ...
    subplot(2,2,3);
    b = bar(M_OV); hold on;
    % Set bar color to a single color (using average of TA and MG)
    avgColor = (colors.TA + colors.MG) / 2;
    b(1).FaceColor = avgColor;
    errorbar(b.XEndPoints, M_OV, SD_OV, 'k', 'LineWidth', 2, 'CapSize', 12, 'LineStyle', 'none');
    ...
    % Apply presentation styling to the figure
    style_figure_for_presentation(gcf());

    exportgraphics(gcf, 'group_comparison_SNR_Duration.pdf', 'ContentType', 'vector');
end
