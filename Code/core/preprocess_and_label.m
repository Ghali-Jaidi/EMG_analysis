...
%% ---- NO scaling - keep raw values for accurate analysis ----
% Scaling is only for visualization purposes and should not affect actual signal values
TT.TA  = TT.TA_raw;
TT.MG  = TT.MG_raw;
TT.Ch3 = TT.Ch3_raw;
...
TT.TA_env = TT.TA_rect;
TT.MG_env = TT.MG_rect;
...
fprintf("Before processing - is_quiet_TA: %d samples\n", sum(is_quiet_TA));
fprintf("min_quiet_samples threshold: %d\n", min_quiet_samples);

% Use quiet mask directly without fuse_masks (which is slow for fragmented masks)
is_rest_TA_unclean = is_quiet_TA;
is_rest_MG_unclean = is_quiet_MG;

fprintf("After direct mask assignment - is_rest_TA_unclean: %d samples\n", sum(is_rest_TA_unclean));
fprintf("After direct mask assignment - is_rest_MG_unclean: %d samples\n", sum(is_rest_MG_unclean));
...
fprintf("is_rest_TA_unclean: %d true samples\n", sum(is_rest_TA_unclean));
fprintf("is_rest_MG_unclean: %d true samples\n", sum(is_rest_MG_unclean));
fprintf("is_valid_acq: %d true samples\n", sum(is_valid_acq));
...
if ~ismember('TA_f', TT_clean.Properties.VariableNames)
    TT_clean.TA_f = TT_clean.TA;
end % Here reput the filters as before later
if ~ismember('MG_f', TT_clean.Properties.VariableNames)
    TT_clean.MG_f = TT_clean.MG;
end
