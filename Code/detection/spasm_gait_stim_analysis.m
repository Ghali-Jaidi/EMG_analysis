function out = spasm_gait_stim_analysis(TT_clean, snrValue, fs, varargin)
...
%
% NAME-VALUE OPTIONS (defaults taken from default_emg_parameters().spasm)
%   'SpasmPrcTA'     : percentile threshold on TA envelope for spasm
%   'SpasmPrcMG'     : percentile threshold on MG envelope for spasm
%   'GaitMinOverlapS': kept for compatibility, not used in current active definition
%   'SpasmMinDurS'   : minimum spasm duration in seconds
%   'FuseGapMs'      : gap in ms to fuse nearby events of same type
%   'Ch3Threshold'   : manual Ch3 ON threshold (auto if empty)
%   'Ch3MinOnMs'     : minimum Ch3 ON duration in ms
%   'PlotResult'     : whether to produce the annotated figure (default false)
%   'PlotDownsample' : downsampling factor for the annotated figure
%   'TitleStr'       : figure title string

% Defaults come from the central config (default_emg_parameters().spasm); any
% of them can still be overridden per call via the name-value options below.
P = default_emg_parameters();

p = inputParser;
p.addParameter('SpasmPrcTA',      P.spasm.prc_TA,            @(x) isnumeric(x) && isscalar(x));
p.addParameter('SpasmPrcMG',      P.spasm.prc_MG,            @(x) isnumeric(x) && isscalar(x));
p.addParameter('GaitMinOverlapS', P.spasm.gait_min_overlap_s,@(x) isnumeric(x) && isscalar(x));
p.addParameter('SpasmMinDurS',    P.spasm.min_dur_s,         @(x) isnumeric(x) && isscalar(x));
p.addParameter('FuseGapMs',       P.spasm.fuse_gap_ms,       @(x) isnumeric(x) && isscalar(x));
p.addParameter('Ch3Threshold',    P.spasm.ch3_threshold,     @(x) isempty(x) || (isnumeric(x) && isscalar(x)));
p.addParameter('Ch3MinOnMs',      P.spasm.ch3_min_on_ms,     @(x) isnumeric(x) && isscalar(x));
p.addParameter('PlotResult',      false,                     @(x) islogical(x) && isscalar(x));
p.addParameter('PlotDownsample',  P.spasm.plot_downsample,   @(x) isnumeric(x) && isscalar(x));
