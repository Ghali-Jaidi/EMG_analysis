...
function [] = plot_filtered_labeled_GUI(channel_1, channel_2, channel_3, t, snrValue, ax)
    % Downsample for faster rendering in GUI (keep every Nth point)
    % Target ~5000 points maximum for smooth but fast rendering
    n = length(channel_1);
    downsample_factor = max(1, floor(n / 5000));
    
    if downsample_factor > 1
        idx = 1:downsample_factor:n;
        channel_1 = channel_1(idx);
        channel_2 = channel_2(idx);
        channel_3 = channel_3(idx);
        t = t(idx);
        
        % Downsample masks too
        if isfield(snrValue, 'is_rest')
            snrValue.is_rest = snrValue.is_rest(idx);
        end
        if isfield(snrValue, 'is_rest_MG')
            snrValue.is_rest_MG = snrValue.is_rest_MG(idx);
        end
        if isfield(snrValue, 'is_act')
            snrValue.is_act = snrValue.is_act(idx);
        end
        if isfield(snrValue, 'is_act_MG')
            snrValue.is_act_MG = snrValue.is_act_MG(idx);
        end
        if isfield(snrValue, 'is_valid')
            snrValue.is_valid = snrValue.is_valid(idx);
        end
    end
    
    plot_filtered_labeled(channel_1, channel_2, channel_3, t, snrValue, ax);
end
