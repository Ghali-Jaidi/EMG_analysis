...
> Figure parameters are centralized too: the annotated spasm figure produced by
> `detection/spasm_gait_stim_analysis` downsamples by `P.spasm.plot_downsample`
> (from `config/default_emg_parameters.m`), overridable via the `PlotDownsample`
> name-value option.

## Related Modules

- **Analysis** (`analysis/`) & **Detection** (`detection/`): generate data and inline figures
- **Config** (`config/`): `default_emg_parameters` (figure knobs) and the GUI (interface.mlapp)
